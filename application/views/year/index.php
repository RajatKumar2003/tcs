    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper" class="gray-bg">
            ...
            <div class="row wrapper border-bottom white-bg page-heading">
                ...
                <div class="col-lg-2">
                <h2><a href="<?php echo site_url('Year') ?>" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Add Year</a></h2>
                </div>
            </div>
                                            ...
                                            if (!empty($yearlist)) {
                                                ...
                                                foreach ($yearlist as $list) {  ?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="<?php echo base_url('Year/') . 'edityear/' . $list->YearId ?>" class="btn btn-success btn-circle"><i class="fa fa-edit"></i>
                                                            </a>

                                                            <button class="btn btn-danger btn-circle" type="button" onclick="showConfirmation('<?php echo $list->YearId ?>')"><i class="fa fa-trash"></i>
                                                            </button>
                                ...
                                <form id="<?php echo ($editYear)? "updateyear":"addyear" ?>" action="<?php echo base_url() . 'UserRoles/add'; ?>" method="post">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="status" id="" class="form-control">
                                            <option value="1" <?php echo ($editYear) ? (($editYear->Status == 1) ? 'selected' : '') : ''; ?>>Active</option>
                                            <option value="0" <?php echo ($editYear) ? (($editYear->Status == 0) ? 'selected' : '') : ''; ?>>Inactive</option>
                                        </select>
                                    </div>

                                    <div class="form-group ">

                                        <button type="submit" id="<?php echo $editYear ? 'update' : 'save'; ?>" class="btn btn-success ladda-button ladda-button-demo" <?php if ($editYear) { ?> formaction='<?php echo base_url('updateemployee') ?>' <?php } ?>>
                                            <i class="fa fa-bookmark"></i> <?php echo $editYear ? 'Update' : 'Add'; ?>
                                        </button>
...
function performAjaxFunction(global_var) {
            ...
            $.ajax({
                url: '<?php echo base_url() . 'Year/deleteyear'; ?>',
                method: 'POST',
                data: {
                    id: global_var
                },
                success: function(data) {
                    ...
                    var $toast = toastr['success']('Year successfully deleted');
                    setTimeout(function() {
                        location.reload();
                    }, 2000)
                },
                error: function(error) {
                    ...
                    var $toast = toastr['error']('Year not  deleted');
    ...
    <?php $this->load->view('Year/yearjs'); ?>
