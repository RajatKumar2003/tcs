    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper" class="gray-bg">
            ...
            <div class="row wrapper border-bottom white-bg page-heading">
                ...
                <div class="col-lg-2">
                <h2><a href="<?php echo site_url('State') ?>" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Add State</a></h2>
                </div>
            </div>
                                            ...
                                            if (!empty($statelist)) {
                                                ...
                                                foreach ($statelist as $list) {  ?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="<?php echo base_url('State/') . 'editstate/' . $list->StateId ?>" class="btn btn-success btn-circle"><i class="fa fa-edit"></i>
                                                            </a>

                                                            <button class="btn btn-danger btn-circle" type="button" onclick="showConfirmation('<?php echo $list->StateId ?>')"><i class="fa fa-trash"></i>
                                                            </button>
                                ...
                                <form id="<?php echo ($editState)? "updatestate":"addstate" ?>" action="<?php echo base_url() . 'UserRoles/add'; ?>" method="post">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="status" id="" class="form-control">
                                            <option value="1" <?php echo ($editState) ? (($editState->Status == 1) ? 'selected' : '') : ''; ?>>Active</option>
                                            <option value="0" <?php echo ($editState) ? (($editState->Status == 0) ? 'selected' : '') : ''; ?>>Inactive</option>
                                        </select>
                                    </div>

                                    <div class="form-group ">

                                        <button type="submit" id="<?php echo $editState ? 'update' : 'save'; ?>" class="btn btn-success ladda-button ladda-button-demo" <?php if ($editState) { ?> formaction='<?php echo base_url('updateemployee') ?>' <?php } ?>>
                                            <i class="fa fa-bookmark"></i> <?php echo $editState ? 'Update' : 'Add'; ?>
                                        </button>
...
function performAjaxFunction(global_var) {
            ...
            $.ajax({
                url: '<?php echo base_url() . 'State/deletestate'; ?>',
                method: 'POST',
                data: {
                    id: global_var
                },
                success: function(data) {
                    ...
                    var $toast = toastr['success']('State successfully deleted');
                    setTimeout(function() {
                        location.reload();
                    }, 2000)
                },
                error: function(error) {
                    ...
                    var $toast = toastr['error']('State not  deleted');
    ...
    <?php $this->load->view('State/statejs'); ?>
